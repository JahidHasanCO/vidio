...
class VideoPlayerConfig {
  ...
  static const double qualityPickerPositionOffset = 35.0;
  static const double qualityPickerWidthDivider = 3.0;

  // Progress Indicator Constants
  static const double miniProgressHeight = 1.0;
  ...
  // Layout Constants
  static const double backButtonMargin = 10.0;
...
class VideoConfigurationManager {
  ...
  double getAspectRatio(bool isFullScreen, double defaultAspectRatio) {
    return isFullScreen ? VideoPlayerConfig.defaultAspectRatio : defaultAspectRatio;
  }

  /// Get the rewind/fast forward threshold positions
  double getRewindThreshold(double width) => width * VideoPlayerConfig.rewindThreshold;
  double getFastForwardThreshold(double width) => width * VideoPlayerConfig.fastForwardThreshold;
  ...
  EdgeInsets getBackButtonMargin(bool isFullScreen) {
    return EdgeInsets.only(
      top: isFullScreen ? VideoPlayerConfig.backButtonMargin : VideoPlayerConfig.backButtonMargin / 2,
      left: isFullScreen ? VideoPlayerConfig.backButtonMargin : VideoPlayerConfig.backButtonMargin / 2,
    );
  }

  /// Get quality picker position
  double getQualityPickerPositionRight(double width) => width / VideoPlayerConfig.qualityPickerWidthDivider;
  double getQualityPickerPositionTop(double offset) => offset + VideoPlayerConfig.qualityPickerPositionOffset;
}